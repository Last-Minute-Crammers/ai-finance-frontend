...
export const userLogin = async (email: string, password: string): Promise<ApiResponse<LoginResponse>> => {
  try {
    __f__('log','at common/api/user.ts:20','Attempting login with:', { email, password: '***' });
    ...
    __f__('log','at common/api/user.ts:28','Login response received:', response);
    __f__('log','at common/api/user.ts:29','Response type:', typeof response);
    ...
    if (isSuccess) {
      ...
      if (token) {
        ...
        __f__('log','at common/api/user.ts:64','Token saved successfully:', token.substring(0, 20) + '...');
        ...
        __f__('error','at common/api/user.ts:83','登录成功但未找到token，完整响应:', JSON.stringify(response, null, 2));
        throw new Error('登录成功但服务器未返回访问令牌');
      }
    } else {
      // 处理登录失败的情况
      const errorMessage = response.Msg || response.message || response.error || response.msg || '登录失败';
      __f__('error','at common/api/user.ts:89','登录失败:', errorMessage, response);
      throw new Error(errorMessage);
    }
    
  } catch (error) {
    __f__('error','at common/api/user.ts:94','Login error details:', error);
...
export const userLogout = () => {
  ...
  __f__('log','at common/api/user.ts:135','User logged out, token cleared');
}
...
export const getFriendConsumption = (friendId: number): Promise<ApiResponse<{
  ...
  return request({
    url: `/api/user/friend/${friendId}/consumption`,
    method: 'GET',
    requireAuth: true
  });
}
...
// 获取好友PK排行榜
export const getFriendRanking = (): Promise<ApiResponse<{
  username: string;
  savings: number;
  rank: number;
}[]>> => {
  return request({
    url: '/api/user/friend/ranking',
    method: 'GET',
    requireAuth: true
  });
}
...
// 给好友发表评论/吐槽
export const postFriendComment = (friendId: number, content: string): Promise<ApiResponse<null>> => {
  return request({
    url: `/api/user/friend/${friendId}/comment`,
    method: 'POST',
    data: { content },
    requireAuth: true
  });
...
// 获取好友评论列表
export const getFriendComments = (friendId: number): Promise<ApiResponse<{
  content: string;
  author: string;
  createTime?: string;
}[]>> => {
  return request({
    url: `/api/user/friend/${friendId}/comments`,
    method: 'GET',
    requireAuth: true
  });
}
...
// 设置存钱目标
export const setSavingsGoal = (data: {
  target: number;
  deadline?: string;
  description?: string;
}): Promise<ApiResponse<null>> => {
