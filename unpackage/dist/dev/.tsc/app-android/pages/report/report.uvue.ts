...
async function generateReport() {
  ...
  try {
    ...
    console.log('开始生成AI报告，类型:', type, " at pages/report/report.uvue:195")
    
    // 调用AI报告API，它会自动获取所有需要的数据
    const aiResponse = await getAIReport({ type })
    
    console.log('AI报告生成响应:', aiResponse, " at pages/report/report.uvue:200")
    ...
    if (aiResponse && (aiResponse.summary || aiResponse.suggestion || aiResponse.tags)) {
      ...
      console.log('AI报告数据处理成功:', { summary: summary.value, suggestion: suggestion.value, tags: tags.value }, " at pages/report/report.uvue:207")
    } else {
      console.log('AI报告数据格式错误:', aiResponse, " at pages/report/report.uvue:209")
    ...
    try {
      ...
      console.log('总统计数据响应:', statsResponse, " at pages/report/report.uvue:216")
      ...
      if (statsResponse) {
        ...
        console.log('总统计数据:', totalStats, " at pages/report/report.uvue:229")
        
        if (totalStats && (totalStats.Income || totalStats.Expense)) {
          updateReportDataWithTotalStats(totalStats)
        } else {
          console.log('没有找到有效的统计数据', " at pages/report/report.uvue:234")
          updateReportDataWithNoData()
        }
      } else {
        console.log('没有找到统计数据，显示默认信息', " at pages/report/report.uvue:238")
        updateReportDataWithNoData()
      }
    } catch (statsError) {
      console.error('获取统计数据失败:', statsError, " at pages/report/report.uvue:242")
      updateReportDataWithNoData()
    }
    ...
    console.error('生成报告失败:', error, " at pages/report/report.uvue:251")
    uni.showToast({
      title: '生成失败，请重试',
      icon: 'none'
    })
...
function updateReportDataWithTotalStats(totalStats: any) {
  console.log('开始处理总统计数据:', totalStats, " at pages/report/report.uvue:268")
  console.log('totalStats类型:', typeof totalStats, " at pages/report/report.uvue:269")
  console.log('totalStats.Income:', totalStats.Income, " at pages/report/report.uvue:270")
  console.log('totalStats.Expense:', totalStats.Expense, " at pages/report/report.uvue:271")
  
  const income = totalStats.Income || { Amount: 0, Count: 0 }
  const expense = totalStats.Expense || { Amount: 0, Count: 0 }
  
  console.log('收入数据:', income, " at pages/report/report.uvue:276")
  console.log('支出数据:', expense, " at pages/report/report.uvue:277")
  console.log('收入Amount:', income.Amount, " at pages/report/report.uvue:278")
  console.log('支出Amount:', expense.Amount, " at pages/report/report.uvue:279")
  ...
  console.log('计算后的金额:', {
    ...
    savings,
    savingsRate
  }, " at pages/report/report.uvue:286")
  ...
  console.log('更新后的报告数据:', reportData.value, " at pages/report/report.uvue:320")
}
...
onMounted(() => {
  ...
  // 调试信息：验证时间计算
  console.log('周报时间范围:', getTimeRange('week'), " at pages/report/report.uvue:336")
  console.log('月报时间范围:', getTimeRange('month'), " at pages/report/report.uvue:337")
  console.log('年报时间范围:', getTimeRange('year'), " at pages/report/report.uvue:338")
})
