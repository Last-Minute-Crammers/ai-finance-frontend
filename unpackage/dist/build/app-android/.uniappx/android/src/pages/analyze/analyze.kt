...
open class GenPagesAnalyzeAnalyze : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesAnalyzeAnalyze) -> Any? = fun(__props): Any? {
            ...
            val chartColors: UTSJSONObject = object : UTSJSONObject() {
                var bar = utsArrayOf(
                    "#91CB74",
                    "#EE6666"
                )
            ...
            fun gen_loadCategoryData_fn(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        try {
                            ...
                            console.log("交易列表响应类型:", UTSAndroid.`typeof`(response))
                            console.log("交易列表响应所有属性:", Object.keys(response))
                            console.log("response.Data:", response.Data)
                            console.log("response.data:", response.data)
                            console.log("response.Msg:", response.Msg)
                            if (response.Data) {
                                console.log("Data 字段类型:", UTSAndroid.`typeof`(response.Data))
