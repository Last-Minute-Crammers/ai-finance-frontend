...
open class GenPagesAIchatAIchat : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesAIchatAIchat) -> Any? = fun(__props): Any? {
            ...
            onMounted(fun(){
                console.log("AI聊天页面初始化", " at pages/AIchat/AIchat.uvue:92")
                val pages = getCurrentPages()
                val currentPage = pages[pages.length - 1]
                console.log("当前页面信息:", currentPage, " at pages/AIchat/AIchat.uvue:96")
                if (currentPage && currentPage.options && currentPage.options["sessionId"]) {
                    val sessionId = currentPage.options["sessionId"]
                    console.log("检测到传入的sessionId:", sessionId, " at pages/AIchat/AIchat.uvue:100")
                    loadSession(sessionId)
                } else {
                    console.log("没有检测到传入的sessionId，显示默认欢迎语", " at pages/AIchat/AIchat.uvue:103")
            ...
            fun gen_loadHistory_fn(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        ...
                        try {
                            ...
                            console.log("历史记录API响应:", res, " at pages/AIchat/AIchat.uvue:121")
                            if (res.success && UTSArray.isArray(res.data)) {
                                ...
                                console.log("处理后的历史记录:", chatHistory.value, " at pages/AIchat/AIchat.uvue:128")
                            } else {
                                chatHistory.value = utsArrayOf()
                                console.warn("历史记录响应格式错误:", res, " at pages/AIchat/AIchat.uvue:131")
                            }
                        }
                         catch (e: Throwable) {
                            console.error("加载历史记录失败:", e, " at pages/AIchat/AIchat.uvue:134")
                            chatHistory.value = utsArrayOf()
                        }
                         finally{
            ...
            fun gen_loadSession_fn(sessionId: String): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        ...
                        console.log("开始加载会话:", sessionId, " at pages/AIchat/AIchat.uvue:146")
                        try {
                            val res = await(getAIChatSessionDetail(sessionId))
                            console.log("会话详情API响应:", res, " at pages/AIchat/AIchat.uvue:150")
                            if (res.success && UTSArray.isArray(res.data)) {
                                val sorted = res.data.slice().sort(fun(a, b){
                                    return Date(a.createdAt).getTime() - Date(b.createdAt).getTime()
                                })
                                console.log("排序后的会话数据:", sorted, " at pages/AIchat/AIchat.uvue:155")
                                messages.value = utsArrayOf()
                                sorted.forEach(fun(item: Any, index: Number){
                                    console.log("\u5904\u7406\u7B2C" + (index + 1) + "\u6761\u8BB0\u5F55:", item, " at pages/AIchat/AIchat.uvue:161")
                                    console.log("- input: \"" + item.Input + "\"", " at pages/AIchat/AIchat.uvue:162")
                                    console.log("- response: \"" + item.Response + "\"", " at pages/AIchat/AIchat.uvue:163")
                                    if (item.Input && item.Input.trim() !== "") {
                                        messages.value.push(object : UTSJSONObject() {
                                            var role = "user"
                                            var text = item.Input
                                        })
                                        console.log("\u6DFB\u52A0\u7528\u6237\u6D88\u606F: \"" + item.Input + "\"", " at pages/AIchat/AIchat.uvue:168")
                                    }
                                    if (item.Response && item.Response.trim() !== "") {
                                        ...
                                        console.log("\u6DFB\u52A0AI\u6D88\u606F: \"" + item.Response + "\"", " at pages/AIchat/AIchat.uvue:173")
                                    }
                                })
                                console.log("处理完成后的消息数组:", messages.value, " at pages/AIchat/AIchat.uvue:177")
                                if (messages.value.length === 0) {
                                    console.log("没有找到历史消息，显示默认欢迎语", " at pages/AIchat/AIchat.uvue:181")
                                    messages.value.push(object : UTSJSONObject() {
                                        var role = "ai"
                                        var text = "你好，我是理财小汪，你的智能理财助手，有什么可以帮您？"
                                    })
                                } else {
                                    console.log("成功加载历史消息，消息数量:", messages.value.length, " at pages/AIchat/AIchat.uvue:184")
                                }
                                scrollToBottom()
                                console.log("成功加载会话:", sessionId, "消息数量:", messages.value.length, " at pages/AIchat/AIchat.uvue:188")
                            } else {
                                messages.value = utsArrayOf(
                                    object : UTSJSONObject() {
                                        var role = "ai"
                                ...
                                console.error("加载会话失败: 响应数据格式错误", res, " at pages/AIchat/AIchat.uvue:192")
                            }
                        }
                         catch (e: Throwable) {
                            console.error("加载会话失败:", e, " at pages/AIchat/AIchat.uvue:195")
                            messages.value = utsArrayOf(
                                object : UTSJSONObject() {
                                    var role = "ai"
                                    var text = "加载历史对话失败，请稍后重试。"
            ...
            fun gen_sendMessage_fn(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w@{
                        ...
                        try {
                            console.log("发送消息，当前sessionId:", currentSessionId.value, " at pages/AIchat/AIchat.uvue:216")
                            ...
                            if (response.success && response.data) {
                                ...
                                if (!currentSessionId.value) {
                                    ...
                                    if (chatHistory.value.length > 0) {
                                        ...
                                        console.log("获取到新的sessionId:", currentSessionId.value, " at pages/AIchat/AIchat.uvue:229")
                                    }
                                }
                            } else {
                            ...
                            console.error("发送消息失败:", error, " at pages/AIchat/AIchat.uvue:237")
                            messages.value.splice(loadingMsgIndex, 1)
                            messages.value.push(object : UTSJSONObject() {
                                var role = "ai"
