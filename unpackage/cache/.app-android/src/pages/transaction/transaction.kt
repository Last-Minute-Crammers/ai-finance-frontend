    ...
    companion object {
        ...
        var setup: (__props: GenPagesTransactionTransaction) -> Any? = fun(__props): Any? {
            ...
            val fetchCategoryList = fun(forceRefresh: Boolean = false): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        try {
                            ...
                            console.log("分类列表响应:", response, " at pages/transaction/transaction.uvue:155")
                            ...
                            }, " at pages/transaction/transaction.uvue:156")
                            if (response.code === 200) {
                                categories.value = response.data || utsArrayOf()
                                selectedCategoryId.value = null
                                if (response.message === "success (cached)") {
                                    console.log("使用本地缓存数据", " at pages/transaction/transaction.uvue:172")
                                } else {
                                    console.log("使用服务器数据", " at pages/transaction/transaction.uvue:174")
                                }
                                if (categories.value.length === 0) {
                                    console.log("当前类型暂无分类数据", " at pages/transaction/transaction.uvue:179")
                                } else {
                                    console.log("成功加载分类数据:", categories.value.length, "个分类", " at pages/transaction/transaction.uvue:181")
                                }
                            } else {
                                console.error("API返回错误:", response, " at pages/transaction/transaction.uvue:184")
                                uni_showToast(ShowToastOptions(title = response.message || "获取分类列表失败", icon = "none"))
                            }
                        }
                         catch (error: Throwable) {
                            console.error("获取分类列表失败:", error, " at pages/transaction/transaction.uvue:191")
            ...
            val refreshCategories = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        try {
                            ...
                            console.error("刷新分类失败:", error, " at pages/transaction/transaction.uvue:240")
            ...
            val onCategoryCreated = fun(newCategory: Category){
                console.log("新分类已创建:", newCategory, " at pages/transaction/transaction.uvue:255")
            ...
            fun gen_submitTransaction_fn(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w@{
                        try {
                            ...
                            val transactionData: UTSJSONObject = object : UTSJSONObject(UTSSourceMapPosition("transactionData", "pages/transaction/transaction.uvue", 340, 11)) {
