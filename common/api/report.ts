...
async function getCompleteStatistics(type: 'week' | 'month' | 'year') {
  ...
  console.log('获取统计数据，类型:', type, '时间范围:', { startTime, endTime })
  ...
  try {
    ...
    console.log('转换前基础统计:', baseStats?.Data || baseStats?.data || baseStats)
    console.log('转换后基础统计:', baseStatsConverted)
    console.log('转换前收入类别统计:', incomeCategoryStats?.Data || incomeCategoryStats?.data || incomeCategoryStats)
    console.log('转换后收入类别统计:', incomeCategoryStatsConverted)
    console.log('转换前支出类别统计:', expenseCategoryStats?.Data || expenseCategoryStats?.data || expenseCategoryStats)
    console.log('转换后支出类别统计:', expenseCategoryStatsConverted)
    ...
    console.log('完整统计数据（已转换为元）:', completeStats)
    console.log('原始基础统计数据:', baseStats)
    console.log('原始收入类别统计:', incomeCategoryStats)
    console.log('原始支出类别统计:', expenseCategoryStats)
    console.log('原始细粒度数据:', detailStats)
    
    // 验证转换结果
    if (completeStats.base_statistics) {
      console.log('转换后的基础统计:', completeStats.base_statistics)
    }
    if (completeStats.income_category_stats) {
      console.log('转换后的收入类别统计:', completeStats.income_category_stats)
    }
    if (completeStats.expense_category_stats) {
      console.log('转换后的支出类别统计:', completeStats.expense_category_stats)
    }
...
function testConversion() {
  ...
  console.log('测试转换:', testData, '->', converted)
} 